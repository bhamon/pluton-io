...
function factory(_server, _config) {
  ...
  function read(_stream, _localConfig) {
    const localConfig = extend(true, {}, config, _localConfig);
    return new Promise((_resolve, _reject) => {
      const contentLength = _stream.in[http2.constants.HTTP2_HEADER_CONTENT_LENGTH] || NaN;
      if (contentLength > localConfig.limit) {
      ...
      function onData(_chunk) {
        if (buffer.length + _chunk.length > localConfig.limit) {
          off();
          return _reject(new Error('Limit exceeded'));
        }
  ...
  _server.decorateStream('body', async function(_localConfig) {
    const raw = await read(this, _localConfig);
