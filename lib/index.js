...
function factory(_server, _config) {
  ...
  function read(_stream) {
    return new Promise((_resolve, _reject) => {
      const contentLength = _stream.in[HEADER_CONTENT_LENGTH] || NaN;
  ...
  function parse(_stream, _raw) {
    const contentType = _stream.in[HEADER_CONTENT_TYPE] || modelMimeType.BINARY;
  ...
  function send(_stream, _data) {
    let contentType = _stream.out[HEADER_CONTENT_TYPE];
    if (!contentType) {
    ...
    _stream.out[HEADER_CONTENT_TYPE] = contentType;
    _stream.out[HEADER_CONTENT_LENGTH] = buffer.length;
  ...
  _server.decorateStream('type', function(_type) {
    this.out[HEADER_CONTENT_TYPE] = _type;
    return this;
  });
